...
(function(){
  ...
  function testReply(){
    ...
    const msg='Ich bin gerade im Testmodus. Deine Nachricht wurde nicht versendet. 💙';
    const status=document.getElementById('johann-test-status');
    if(status)status.textContent=msg;
    // Use Johann's existing speech bubble, not a server request or saved chat.
    if(typeof say==='function')say(msg,8500);
    else {
      let note=document.getElementById('johann-test-toast');
      if(!note){note=document.createElement('div');note.id='johann-test-toast';note.setAttribute('role','status');note.style.cssText='position:fixed;bottom:110px;left:5%;right:5%;z-index:99999;background:#123452;color:white;padding:14px;border-radius:12px;text-align:center';document.body.append(note)}
  ...
  document.addEventListener('click',function(e){
    ...
    if(el.id==='sendButton'){
      e.preventDefault();e.stopImmediatePropagation();testReply();return;
    }
    if(el.matches('[data-kitchen-ask], .kitchen-ask, #mdReload, #mdMailTest, #mdImport, #kidsGenerate, #authRequest, #authVerify')){
      e.preventDefault();e.stopImmediatePropagation();
      const status=document.getElementById('johann-test-status');
      const msg='Testmodus: Diese Funktion benötigt Johann-KI und ist hier absichtlich deaktiviert.';
      if(status)status.textContent=msg;
      if(typeof say==='function')say(msg,7000);
    }
  },true);
  document.addEventListener('keydown',function(e){
    if(e.key==='Enter'&&e.target&&e.target.id==='command'){
      e.preventDefault();e.stopImmediatePropagation();testReply();
    }
  },true);
})();
